/**
 * Spresense_DigDug_proto3_plusbutton.ino
 ...
 * Defs for button
 ****************************************************************************/
#define TRIGGER_BUTTON_PIN  19
#define LED_TRIGGER_PIN     18
#define ALT_TRIGGER_BUTTON_PIN A0
static const uint8_t pin_l = PIN_A0;
int gLastButtonRead = 0;
bool gTriggerOn = false;
uint32_t gImageSetID = 0;
File configIni;
...
void setup()
{
  ...
  */
  ...
  Serial.println("Set the HDR mode");
  err = theCamera.setHDR(CAM_HDR_MODE_ON);
  if (err != CAM_ERR_SUCCESS) {
    printError(err);
  }

  Serial.println("Set the Auto Exposure mode");
  err = theCamera.setAutoExposure(true);
  if (err != CAM_ERR_SUCCESS) {
    printError(err);
  }

  Serial.println("Set the WB mode");
  err = theCamera.setAutoWhiteBalance(true);
  if (err != CAM_ERR_SUCCESS) {
    printError(err);
  }

  Serial.println("Set the AutoISO mode");
  err = theCamera.setAutoISOSensitivity(false);
  if (err != CAM_ERR_SUCCESS) {
    printError(err);
  }

  Serial.println("Set the iso speed");
  err = theCamera.setISOSensitivity(1600 * 1000);
  if (err != CAM_ERR_SUCCESS) {
    printError(err);
  }
  ...
  // pinMode(ALT_TRIGGER_BUTTON_PIN, INPUT);
  // pinMode(ALT_TRIGGER_BUTTON_PIN, INPUT_PULLUP);
  // digitalWrite(ALT_TRIGGER_BUTTON_PIN, LOW);
  // pinMode(ALT_TRIGGER_BUTTON_PIN, INPUT_PULLDOWN);
  // pinMode(A0, INPUT_PULLUP);

  // performImageSetIDUpdate();
}

void performImageSetIDUpdate() {
  configIni = theSD.open("config.ini", FILE_WRITE);

  String dataString = "";
  if (configIni) {
    configIni.seek(0);
    while (configIni.available()) {
      dataString += (char)configIni.read();
    }
    // Initialize the file
    dataString.trim();
    if (dataString.length() == 0) {
      Serial.println("No data in the configIni, initialize to 0");
      configIni.seek(0);
      configIni.println("0");
    } else {
      gImageSetID = dataString.toInt();
      gImageSetID++;
      configIni.seek(0);
      configIni.println(String(gImageSetID));
      Serial.println("Current data in gImageSetID = " + String(gImageSetID));
    }
    configIni.close();
  } else {
    Serial.println("Unable to open the config.ini file");
  }
  theSD.mkdir("set_" + String(gImageSetID));
}
/****************************************************************************
...
void takePicture()
{
  ...
  if (img.isAvailable()) {
    sprintf(filename, "set_%d/PICT%04d_%zu.%s", gImageSetID, g_pict_id++,rtc.unixtime(),
    ...
    Serial.printf("Save: %s in %.2fms\n", filename, averageDuration);
...
/****************************************************************************
 * API: takeRapidFirePicture()
 ****************************************************************************/
void takeRapidFirePicture()
{
  double durations[10] = {0};
  char filename[32] = {0};
  Serial.println();
  Serial.println("--- Take Rapid Fire Picture ---");
  ...
  for (int i = 0; i < 10; i++) {
    ...
    RtcTime rtc = RTC.getTime();
    if (img.isAvailable()) {
      sprintf(filename, "PICT%04d%d.%s", g_pict_id++,rtc.nsec(),
              (img.getPixFormat() == CAM_IMAGE_PIX_FMT_RGB565) ? "RGB565" :
              (img.getPixFormat() == CAM_IMAGE_PIX_FMT_YUV422) ? "YUV422" :
              (img.getPixFormat() == CAM_IMAGE_PIX_FMT_JPG)    ? "JPG" :
              (img.getPixFormat() == CAM_IMAGE_PIX_FMT_GRAY)   ? "GRAY" : "NONE");
  ...
  // Print results
  for (int j = 0; j < 10; j++) {
      Serial.printf("Save: photo %d in %.2fms\n", j + 1, durations[j]);
  }
}
...
void menuTop()
{
  ...
  Serial.println("R    : Rapid Fire Picture (10)");
  Serial.println();
  ...
  else if (input == "R")    { takeRapidFirePicture();     }
}
...
void loop()
{
  ...
  int data = analogRead(ALT_TRIGGER_BUTTON_PIN);
  // Serial.print(">>>> Data: ");
  // Serial.println(data);
  if (data > 900) { // BUtton Pressed
    Serial.println(">>>>>>>>>>>>>>>>TRIGGER BUTTON PRESSED");     // send this Character
  }
  if (gLastButtonRead >= 900 && data < 900) { // BUtton Released
    Serial.println(">>>>>>>>>>>>>>>>TRIGGER BUTTON RELEASED");
    if (gTriggerOn) {
      digitalWrite(LED_TRIGGER_PIN, LOW);
      gTriggerOn = false;
      // Move the set into a numbered set to the next set
      // This avoids destructive overwrites since the RTC doesn't work between runs
    } else {
      performImageSetIDUpdate();
  ...
  gLastButtonRead = data;

  // Take a picture ... todo figure out a better way to loop
  if (gTriggerOn) {
    takePicture();
    // takeRapidFirePicture();
    delay(50);
  } 
